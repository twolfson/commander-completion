  ...
  complete: function (params, cb) {
    ...
    function addChildren(root) {
      ...
      /*
        {
          'hello': {
            '-n': function (info, cb) {
              var remainingWords = info.something;
              // TODO: We should combine each of the inclusive/exclusive variants
              var optionLessWords = remainingWords.popRemainingFilledOutOptionValues;
              // TODO: Complete should run on the subtree (e.g. `world`)
              return this.complete(optionLessWords, cb);
            },
            'world': function (info, cb) {
      ...
      */
      ...
      // If there was no completion logic
      if (retVal === undefined) {
        // Recurse each of the commands as a child
        retVal = {};
        root.commands.forEach(function addCommand (command) {
          // Add the command to our list
          var name = command._name;
          retVal[name] = addChildren(command);

          // Additionally, if there are any options on that command, add those as viable venues
          // TODO: What if there are commands with the same options but one takes 0 and the other requires 1?
          command.options.forEach(function addCommandOption (option) {
            // TODO: Only add to `short` and `long ` as they exist
            retVal[option.short] = function (info, cb) {
              var remainingLeftWords = info.words.remainingLeftWords;
              return this.complete(remainingLeftWords, cb);
            };
          });
        });
      }
