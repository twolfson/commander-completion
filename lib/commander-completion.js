...
var CompletableProto = {
  ...
  complete: function (params, cb) {
    ...
    function getCompletionTree(command) {
      ...
      var retVal = {
        ...
        name: command.name || command._name
      };

      // If there is completion logic, save it
      if (command._completion) {
        retVal.completion = command._completion;
      // Otherwise
      } else {
        // Recurse each of the commands as a child
        retVal.commands = command.commands.map(function addCommand (command) {
